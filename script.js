// Authentic Indian Restaurant Data
const restaurants = [
    // --- HIGHLY RATED ---
    {
        name: "Indian Accent",
        ...
        desc: "World-renowned modern Indian culinary masterpiece. Famous for its inventive tatte-style courses and spectacular blue cheese naan.",
    ...
    // --- MOST POPULAR ---
    {
        ...
        meta: "50k+ Happy Customers Weekly | South Indian QSR"
    },
    {
        name: "Britannia & Co.",
        location: "📍 Mumbai",
    ...
    {
        name: "Paranthe Wali Gali (Babu Rao)",
        location: "📍 Old Delhi",
        category: "popular",
...
function filterRestaurants(category) {
    // Dynamically update visual tab styles
    const buttons = document.querySelectorAll(".tab-btn");
    buttons.forEach(btn => btn.classList.remove("active"));
    
    // Safety check for user click event handling
    if(window.event && window.event.target) {
        window.event.target.classList.add("active");
    }

    // Filter array to match chosen tab
    const filtered = restaurants.filter(r => r.category === category);
    displayRestaurants(filtered);
}

// Automatically load the highly-rated category first when page opens
document.addEventListener("DOMContentLoaded", () => {
    // Find the first button and ensure it stays styled properly
